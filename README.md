...
Uploaded workbook bytes are read in the browser. This project has no server route for receiving transaction data and no application database. Users should still avoid deploying modified forks that add third-party scripts without understanding their data access.
...
- No persistent local rule store yet; recategorization applies for the current browser session
